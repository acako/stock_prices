...
```{r missing values, echo=FALSE, message=FALSE, fig.cap='Missing values map: before and after imputation'}
...
## Data Distribution
we then performed the distribution plot for all the columns to check the data distribution, to observe the means, and check for outliers. Most variables were not normally distributed and had a clear skew. In Figure (FILL_IN), we show a subset of the variable distributions.
...
``````{r correlation plot 2, echo=FALSE,fig.align='center'}
...
```{r cluster sector echo = FALSE, message = FALSE, fig.cap = 'Clusters by Sector', fig.align='center', fig.height=5,fig.width=5}
ggplot(df_updated, aes(x=cluster, fill = Sector)) + geom_bar(position = 'fill')
```

```{r cluster tables, echo = FALSE}
c1df <- left_join(unique(df_c1), stocks_lookup_table, by = 'X')
c1df <- c1df[, names(c1df) %in% c('X','name','Sector','Market.Cap')]
c1df <- c1df[,c(1,4,3,2)]
c2df <- left_join(unique(df_c2), stocks_lookup_table, by = 'X')
c2df <- c2df[, names(c2df) %in% c('X','name','Sector','Market.Cap')]
c2df <- c2df[,c(1,4,3,2)]
c3df <- left_join(unique(df_c3), stocks_lookup_table, by = 'X')
c3df <- c3df[, names(c3df) %in% c('X','name','Sector','Market.Cap')]
c3df <- c3df[,c(1,4,3,2)]
c4df <- left_join(unique(df_c4), stocks_lookup_table, by = 'X')
c4df <- c4df[, names(c4df) %in% c('X','name','Sector','Market.Cap')]
c4df <- c4df[,c(1,4,3,2)]
```
```{r top companies in each cluster, echo = FALSE}
sortedc1 <- na.omit(c1df)
sortedc1 <- sortedc1[order(-sortedc1$Market.Cap),]
sortedc1 <- sortedc1[1:15,]
c1table <- sortedc1[, !names(sortedc1) %in% c('Market.Cap')]
c1table <- unique(c1table)

sortedc2 <- na.omit(c2df)
sortedc2 <- sortedc2[order(-sortedc2$Market.Cap),]
sortedc2 <- sortedc2[1:30,]
c2table <- sortedc2[, !names(sortedc2) %in% c('Market.Cap')]
c2table <- unique(c2table)
c2table <- c2table[1:7,]

sortedc3 <- na.omit(c3df)
sortedc3 <- sortedc3[order(-sortedc3$Market.Cap),]
sortedc3 <- sortedc3[1:30,]
c3table <- sortedc3[, !names(sortedc3) %in% c('Market.Cap')]
c3table <- unique(c3table)

sortedc4 <- na.omit(c4df)
sortedc4 <- sortedc4[order(-sortedc4$Market.Cap),]
sortedc4 <- sortedc4[1:30,]
c4table <- sortedc4[, !names(sortedc4) %in% c('Market.Cap')]
c4table <- unique(c4table)
c4table <- c4table[1:10,]
```
...
#### **Random Forest**\
```{r random forest, message = FALSE, warning = FALSE, echo = FALSE,fig.height=5, fig.width=5,fig.align='center'}
...
The model used tuning parameter 'subsample' as a constant at a value of 0.8, and RMSE was used to select the optimal model using the smallest value.
The final values used for the model were nrounds = 200, max_depth =
 6, eta = 0.1, gamma = 0, colsample_bytree = 0.5, min_child_weight =
 1 and subsample = 0.8.
```{r XGBoost, message = FALSE, warning = FALSE, echo = FALSE,fig.height=5, fig.width=5,fig.align='center'}
...
```{r Lasso regression, message = FALSE, warning = FALSE, echo = FALSE,fig.height=5, fig.width=5,fig.align='center'}
Lasso_Regression_Model <- readRDS("Lasso_Model.rds")
ggplot2::ggplot(varImp(Lasso_Regression_Model))
...
RMSE<- c(274957.8,10811777965,14502322435,11765977743)
R2 <- c(0.8067,0.9010295,0.8229718,0.8754267)
MAE <- c(135701,2699512435,3748806832,2917376718)
