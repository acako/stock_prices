...
####Varaible Importancy
We decided to use decision tree to check the variable importancy as a important reference for us to do feature selection.
```{r}
...
barplot(dTreeImp, col = 'blue', xlab = 'Relative Influence', horiz = TRUE, las = 2, names = model_importance, main = 'Variable Importance', cex = 0.5, cex.main = 0.8, cex.lab = 0.8, cex.axis = 0.6)
```


We also did some data visualization for our final data set we will use for modeling.
```{r text summary, echo=FALSE}
full_set <- subset(full_set[,c(2,16,43,35,10,31,36,8,6,28,27,47)])
write.csv(full_set,'important.csv')

summary(df_numeric)
table(df_numeric$type)
```
