...
set.seed(123)
complete_data$Sector <- as.numeric(as.factor(complete_data$Sector))
imputed_data_cart <- mice(complete_data, m=5, method='cart',maxit = 5)
imputed_data_rf <- mice(complete_data, m=2, method='rf',maxit = 2)
...
#complete_cart$Sector <- as.numeric(as.factor(complete_cart$Sector))
complete_cart_pca <- subset(complete_cart[,2:222])
cart_pca <- prcomp(complete_cart_pca, scale=TRUE)
